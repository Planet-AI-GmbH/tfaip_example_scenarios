...
from tfaipscenarios.atr.model import ATRModelParams
...
class ATRGraph(GraphBase[ATRModelParams]):
    def __init__(self, params: ATRModelParams, **kwargs):
        ...
        self.bilstm_layer = keras.layers.Bidirectional(keras.layers.LSTM(self._params.lstm_nodes, return_sequences=True))
    ...
    def call(self, inputs, **kwargs):
        ...
        greedy_decoded = ctc_ops.ctc_greedy_decoder(inputs=tf.transpose(blank_last_logits, perm=[1, 0, 2]),
                                                    sequence_length=tf.cast(keras.backend.flatten(data_length),
                                                                            'int32'))[0][0]
